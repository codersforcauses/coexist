...
>(({ className, children, wrapHeight = false, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        `fixed left-[50%] top-[50%] z-50 flex w-full -translate-x-1/2 -translate-y-1/2 flex-col gap-4 overflow-y-auto border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] md:w-4/5 md:rounded-lg lg:w-3/5 2xl:w-2/5`,
        wrapHeight ? "max-h-full" : "h-full md:h-4/5 lg:h-3/5",
        className,
      )}
      {...props}
    >
